...
class GaussRenderer(nn.Module):
    ...
    def __init__(self, active_sh_degree=3, white_bkgd=True, **kwargs):
        ...
        self.pix_coord = torch.stack(
            torch.meshgrid(torch.arange(256), torch.arange(256), indexing="xy"), dim=-1
        ).to("cuda")
    ...
    def render(self, camera, means2D, cov2d, color, opacity, depths, mask):
        ...
        for h in range(0, camera.image_height, TILE_SIZE):
            for w in range(0, camera.image_width, TILE_SIZE):
                ...
                over_br = rect[1][..., 0].clip(max=w + TILE_SIZE - 1), rect[1][
                    ..., 1
                ].clip(max=h + TILE_SIZE - 1)
                in_mask = (over_br[0] > over_tl[0]) & (
                    over_br[1] > over_tl[1]
                )  # 3D gaussian in the tile
                ...
                alpha = (gauss_weight[..., None] * sorted_opacity[None]).clip(
                    max=0.99
                )  # B P 1
                T = torch.cat(
                    [torch.ones_like(alpha[:, :1]), 1 - alpha[:, :-1]], dim=1
                ).cumprod(dim=1)
    ...
    def forward(self, camera, pc, **kwargs):
        ...
        # print(
        #     camera.world_view_transform.requires_grad,
        #     means2D.requires_grad,
        #     cov2d.requires_grad,
        #     color.requires_grad,
        #     opacity.requires_grad,
        #     depths.requires_grad,
