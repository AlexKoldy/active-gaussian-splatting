...
def get_camera(poses, intrinsic_):
    ...
    if poses.dim() < 3:
        return torch.from_numpy(
            np.concatenate(([256, 256], intrinsic.flatten(), poses.flatten())).astype(
                np.float32
            )
        )
    for pose in poses:
        camera = torch.from_numpy(
            np.concatenate(([256, 256], intrinsic.flatten(), pose.flatten())).astype(
                np.float32
            )
        )
...
def read_all(folder, resize_factor=1.0):
    ...
    src_alphas = []
    src_depths = []
    for src_rgb_file, src_pose, intrinsic in zip(
...
def read_image(
    rgb_file, pose, intrinsic_, max_depth, resize_factor=1.0, white_bkgd=True
):
    ...
    if resize_factor != 1:
        ...
        rgb = rearrange(
            resize_fn(rearrange(rgb, "h w c -> 1 h w c"), resize_factor),
            "1 h w c -> h w c",
        )
        depth = rearrange(
            resize_fn(rearrange(depth, "h w -> 1 h w 1"), resize_factor),
            "1 h w 1 -> h w",
        )
        alpha = rearrange(
            resize_fn(rearrange(alpha, "h w -> 1 h w 1"), resize_factor),
            "1 h w 1 -> h w",
        )
